...
public class RandomItemGenerator {
    ...
    private static TeaPot createPremiumTeaPot(int index) {
        ...
        tp.setPrice((int) (tp.getInPrice() * (2 + rand.nextDouble()))); // Giá bán 2-3x giá nhập
        tp.setYearOfRelease(2000 + rand.nextInt(24));
        List<String> authors = new ArrayList<>();
        int authorCount = 3 + rand.nextInt(6);
        for (int j = 0; j < authorCount; j++) {
            authors.add(ARTIST_NAMES[rand.nextInt(ARTIST_NAMES.length)]);
        }
    ...
    private static Rod createProRod(int index) {
        ...
        rod.setPrice((int) (rod.getInPrice() * (1.5 + rand.nextDouble())));
