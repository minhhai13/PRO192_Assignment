...
public class Utils {
    ...
    public static int inputYear(String message) {
        ...
        while (true) {
            try {
                ...
                System.out.println("Year must greater than 1900 and must not exceed the current year. Please try again.");
            } catch (NumberFormatException e) {
                System.out.println("Invalid input! Please enter an integer.");
            }
        }
    ...
    public boolean checkExists(String id) {
        return ItemList.currentInstance.stream().anyMatch(item -> item.getId().equalsIgnoreCase(id));
    }

// Hàm nhập ID duy nhất, lấy danh sách từ ItemList
    public static String inputUniqueId(String msg) {
        String id;
        ItemList itemList = ItemList.getCurrentInstance();
        if (itemList == null) {
            throw new IllegalStateException("ItemList has not been initialized!");
        }
        while (true) {
            id = inputNonEmptyString(msg);
            if (Utils.checkExists(id)) {
                System.out.println("Error: ID '" + id + "' already exists! Please enter a different ID.");
            } else {
